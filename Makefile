...
dockertest:
	docker run --rm -it -v `pwd`:/go/src/github.com/shoce/process-exporter golang:1.26.4  make -C /go/src/github.com/shoce/process-exporter test

dockerinteg:
	docker run --rm -it -v `pwd`:/go/src/github.com/shoce/process-exporter golang:1.26.4  make -C /go/src/github.com/shoce/process-exporter build integ
