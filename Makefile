...
PREFIX                  ?= $(shell pwd)
BIN_DIR                 ?= $(shell pwd)
DOCKER_IMAGE_NAME       ?= shoce/process-exporter
...
dockertest:
	docker run --rm -it -v `pwd`:/go/src/github.com/shoce/process-exporter golang:1.23.8  make -C /go/src/github.com/shoce/process-exporter test

dockerinteg:
	docker run --rm -it -v `pwd`:/go/src/github.com/shoce/process-exporter golang:1.23.8  make -C /go/src/github.com/shoce/process-exporter build integ
